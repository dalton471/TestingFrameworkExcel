...
list of the workbook's fields, and without depending on the JSON
configuration to supply validation conditions (ranges, formats,
etc.). Every column is discovered from the workbook at runtime, the
...
## 10. How to Execute the Framework
...
Nothing about how the framework is invoked changed in this update.

## 11. Input Workbook Requirements

Unchanged: an `.xlsx`/`.xls` file, sheets listed in `sheet_checks`
checked for existence, one header row per data sheet. No requirement
on column names, order, or count.
...
1. **Validation Summary** - every test that ran: Sheet Existence
   Testing, Rule Determination (per discovered column), and Data
   Validation (per column that received a rule).
...
## 14. Fallback Behavior for Unrecognized Columns

Unchanged from the previous version: a column that doesn't reach the
match threshold for any rule is marked Unrecognized, reported as
`SKIPPED` in the Validation Summary, and excluded from data
validation and row-processing totals - never guessed at (section 6).
...
## 16. Before / After Comparison (Same Sample Workbook)

| Metric | Before this change (JSON-supplied ranges/format) | After this change (Python-only conditions) | Explanation |
|---|---|---|---|
| Total Sheets | 5 | 5 | unchanged |
| Sheets Passed / Failed | 5 / 0 | 5 / 0 | unchanged |
| Total Columns Discovered | 14 | 14 | unchanged |
| Total Rows Processed | 40 | 40 | unchanged |
| Total Validations Executed | 32 | 32 | unchanged |
| Total Passed | 18 | 18 | unchanged |
| Total Failed | 13 | 13 | unchanged |
| **Total Failed Cells** | **19** | **17** | `Salary` and `Performance_Score` are whole-number columns and are now classified `numeric_only` instead of `amount`/`number` (section 7); they still correctly catch their non-numeric corrupt value (`"ABC"`, `"XYZ"`) but no longer flag an out-of-range whole number, since no workbook-tailored range exists anymore |
| Report sheets | 3 | 3 | unchanged |

Every other number is identical; the one change is explained above
rather than forced to match, per the requirement that the framework's
actual, honest output should be reported.
