...
CONFIGURATION IS OPTIONAL, AND JSON ONLY *SELECTS* VALIDATIONS - IT
NEVER *DEFINES* THEM. validation_config.json supplies generic
framework metadata: which sheets are expected (sheet_checks), how the
auto-detection mechanism should be tuned (rule_detection), and,
optionally, which of the framework's supported validations to
explicitly call out via a "validations" list (see
resolve_active_validations() below). It contains NO column names and
NO validation conditions - no regex, no ranges, no date formats,
nothing that describes what a value must look like to be valid, and
nothing that changes which validations ultimately run (JSON can only
highlight a subset for reporting purposes - see
resolve_active_validations()). When the JSON file is missing, main.py
calls build_default_config() below instead of failing - it builds the
identical shape of configuration object directly from the workbook's
own discovered sheet names, and the SAME run_validations() pipeline is
used either way. There is only ONE validation engine; JSON-present and
JSON-absent runs both feed it the same kind of configuration object,
just built from a different source.
...
#
# This list is also the framework's AUTHORITATIVE, PUBLIC registry of
# every validation rule it supports (see SELECT/CONFIGURE vs. DEFINE
# below) - JSON never introduces a rule that isn't already one of
# these names, and this is the single place that list is maintained.
...
# Backward-compatible alias - some code/comments still refer to this
# list by its original internal name.
_RULE_PRIORITY = SUPPORTED_VALIDATIONS


def resolve_active_validations(config):
    """
    Determine which validations run this pass, and how each one was
    chosen - explicitly selected in JSON, or automatically added from
    the Python framework's built-in registry.

    JSON is used only to SELECT/CONFIGURE which validations are of
    interest; it never DEFINES a validation (no regex, no ranges, no
    formats - those live only in this module, see the constants and
    functions above). Concretely, JSON may optionally include a
    "validations" list naming a subset of SUPPORTED_VALIDATIONS. Per
    the framework's requirements, the final set that actually runs is
    always the complete built-in registry - JSON can never narrow
    what runs, only clarify which parts of it were explicitly
    requested versus picked up automatically. This keeps behavior
    identical whether JSON is present, present-but-partial, or absent
    entirely (see main.py's two execution modes).

    Parameters
    ----------
    config : dict
        The active configuration (from JSON, or from
        build_default_config() when JSON is absent).

    Returns
    -------
    dict with:
        "selected_from_json"   : validations explicitly listed in
                                   config["validations"] that are
                                   recognized by this framework, in
                                   the order they were listed.
        "auto_added"            : SUPPORTED_VALIDATIONS entries not
                                   present in selected_from_json, in
                                   this module's canonical order.
        "final_active"          : the complete list actually executed
                                   this run - always equal to
                                   SUPPORTED_VALIDATIONS.
        "unrecognized_requested": entries in config["validations"]
                                   that are not a supported validation
                                   name (reported, never executed,
                                   never crashes the run).
    """
    requested = config.get("validations", [])
    if not isinstance(requested, list):
        requested = []

    seen = set()
    selected_from_json = []
    unrecognized_requested = []

    for name in requested:
        if name in seen:
            continue
        seen.add(name)
        if name in SUPPORTED_VALIDATIONS:
            selected_from_json.append(name)
        else:
            unrecognized_requested.append(name)

    auto_added = [
        rule for rule in SUPPORTED_VALIDATIONS
        if rule not in selected_from_json
    ]

    return {
        "selected_from_json": selected_from_json,
        "auto_added": auto_added,
        "final_active": list(SUPPORTED_VALIDATIONS),
        "unrecognized_requested": unrecognized_requested,
    }
...
def determine_rule_for_column(series, match_threshold, sample_size, active_validations):
    """
    ...
    active_validations : list[str]
        The validations to try, in priority order (normally
        SUPPORTED_VALIDATIONS in its canonical order - see
        resolve_active_validations()). Trying only this list, rather
        than reaching for the module-level registry directly, is what
        lets a future caller narrow or reorder the active rule set
    ...
    for rule_name in active_validations:
...
def discover_and_determine_rules(config, sheets_data, missing_sheets, active_validations):
    """
    ...
    ----------
    active_validations : list[str]
        The validations to try for every column, from
        resolve_active_validations()["final_active"].
    ...
    for sheet_name, df in sheets_data.items():
        ...
        for column_name in discovered_columns:
            rule_name, match_rate, sample_count = determine_rule_for_column(
                df[column_name], match_threshold, sample_size, active_validations
            )
...
def run_validations(config, workbook_sheet_names, sheets_data):
    ...
    ----------
    config : dict
        Full JSON configuration (sheet_checks, rule_detection, and
        optionally "validations" - a SELECTION of validation names,
        never validation logic/conditions themselves).
    workbook_sheet_names : list[str]
    ...
    summary_rows : list[dict]
        ...
        determination, and data validation), PASS/FAIL only.
    detailed_failures : list[dict]
    ...
    validation_selection : dict
        The selected/auto-added/final breakdown from
        resolve_active_validations() - exposed so callers can display
        it (e.g. in the terminal and Execution Summary) without
        recomputing it separately.
    """
    validation_selection = resolve_active_validations(config)
    ...
    rule_summary, column_rules = discover_and_determine_rules(
        config, sheets_data, missing_sheets, validation_selection["final_active"]
    )
    ...
    return summary_rows, detailed_failures, column_rules, validation_selection
