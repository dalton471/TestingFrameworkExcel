...
previous KPI Automation Framework:
    ...
    6. Generate the four-sheet Validation_Report.xlsx.
...
def build_execution_stats(summary_rows, sheets_data, workbook_sheet_names, config):
    ...
    # Total rows processed = data rows across every sheet that has
    # configured fields (Read Me / reference-only sheets are excluded
    # since no data validation runs against them).
    fields_config = config.get("fields", {})
    total_rows_processed = sum(
        len(sheets_data[name]) for name in fields_config
    ...
    total_failed_cells = sum(
        r["Failed Count"] for r in data_rows
    )
...
def main():
    ...
    try:
        summary_rows, detailed_failures = run_validations(
    ...
    execution_stats = build_execution_stats(
        summary_rows, sheets_data, workbook_sheet_names, config
    )
