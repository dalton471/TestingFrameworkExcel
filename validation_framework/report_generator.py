...
Report structure:
    Sheet 1: Validation Summary   - every test executed (pass + fail),
                                     Sheet Name / Field / Test Type /
                                     Test Name / Status / Failed Count.
    Sheet 2: Validation Details   - the same tests with additional
                                     detail: records checked, passed,
                                     failed, and the rule parameters
                                     that were applied.
    Sheet 3: Failed Records       - one row per individual failing
                                     cell across the whole workbook.
    Sheet 4: Execution Summary    - run-level totals and status.
"""
...
def _format_parameters(config, test_type, test_name):
    """Build a short human-readable string of the rule parameters that
    were applied for a given test, sourced from JSON."""
    if test_type != "Data Validation":
        return ""

    rules = config.get("column_checks", {}).get(test_name, {})
    if not rules:
        return "-"

    parts = [f"{key}={value}" for key, value in rules.items()]
    return ", ".join(parts)


def _write_details_sheet(workbook, config, summary_rows):
    sheet = workbook.create_sheet("Validation Details")

    headers = [
        "Sheet Name", "Field", "Test Type", "Test Name",
        "Rule Parameters", "Total Checked", "Passed Count",
        "Failed Count", "Status",
    ]
    _write_header(sheet, headers)

    for row_index, row in enumerate(summary_rows, start=2):
        total_checked = row.get("Total Checked", "-")
        passed_count = row.get("Passed Count", "-")
        ...
        sheet.cell(
            ...
            value=_format_parameters(config, row["Test Type"], row["Test Name"])
        )
        sheet.cell(row=row_index, column=6, value=total_checked)
        sheet.cell(row=row_index, column=7, value=passed_count)
        sheet.cell(row=row_index, column=8, value=row["Failed Count"])
        status_cell = sheet.cell(row=row_index, column=9, value=row["Status"])
    ...
    _add_table(sheet, "ValidationDetails", len(summary_rows), len(headers))
    _autofit_columns(sheet)


# --------------------------------------------------------------------------
# Sheet 3: Failed Records
# --------------------------------------------------------------------------
...
def generate_report(config, summary_rows, detailed_failures, execution_stats, output_file):
    """
    Write the four-sheet Validation_Report.xlsx workbook.

    Parameters
    ----------
    config : dict
        The JSON configuration used for this run (needed to display
        rule parameters in the Validation Details sheet).
    summary_rows : list[dict]
        Every test executed (sheet existence, field existence, data
        validation), from validate.run_validations().
    ...
    _write_details_sheet(workbook, config, summary_rows)
